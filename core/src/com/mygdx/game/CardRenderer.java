...
public class CardRenderer {
	...
	public void renderPlayerCard(float delta) {
		...
		for (int i = 0; i < card.size(); i++) {
			batch.draw(textureRegion[card.get(i).getValue() + 1], 400 - 20 * card.size() + 40 * i, 50, 0, 0, 40, 50, 1,
					1, 0);
		}
		for (int i = 0; i < choosedCard.size(); i++) {
			batch.draw(textureRegion[choosedCard.get(i).getValue() + 1], 400 - 20 * choosedCard.size() + 40 * i, 125, 0,
					0, 40, 50, 1, 1, 0);
		}
		batch.end();
	}
	...
	public void renderBotCard(float delta) {
		...
		SpriteBatch batch = slaveCardGame.batch;
		batch.begin();
		if (world.getPlayer(1).isShowCard()) {
			for (int i = bot1Card.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[bot1Card.get(i).getValue() + 1], 50, 270 + 10 * bot1Card.size() - 20 * i, 20,
						20, 40, 50, 1, 1, -90);
			}
			for (int i = bot1ChoosedCard.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[bot1ChoosedCard.get(i).getValue() + 1], 125,
						270 + 10 * bot1ChoosedCard.size() - 20 * i, 20, 20, 40, 50, 1, 1, -90);
			}
		} else {
			for (int i = bot1Card.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[0], 50, 270 + 10 * bot1Card.size() - 20 * i, 20, 20, 40, 50, 1, 1, -90);
			}
			for (int i = bot1ChoosedCard.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[0], 125, 270 + 10 * bot1ChoosedCard.size() - 20 * i, 20, 20, 40, 50, 1, 1,
						-90);
			}
		}
		if (world.getPlayer(2).isShowCard()) {
			for (int i = bot2Card.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[bot2Card.get(i).getValue() + 1], 370 + 10 * bot2Card.size() - 20 * i, 500, 20,
						25, 40, 50, 1, 1, 180);
			}
			for (int i = bot2ChoosedCard.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[bot2ChoosedCard.get(i).getValue() + 1],
						370 + 10 * bot2ChoosedCard.size() - 20 * i, 425, 20, 25, 40, 50, 1, 1, 180);
			}
		} else {
			for (int i = bot2Card.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[0], 370 + 10 * bot2Card.size() - 20 * i, 500, 20, 25, 40, 50, 1, 1, 180);
			}
			for (int i = bot2ChoosedCard.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[0], 370 + 10 * bot2ChoosedCard.size() - 20 * i, 425, 20, 25, 40, 50, 1, 1,
						180);
			}
		}
		if (world.getPlayer(3).isShowCard()) {
			for (int i = bot3Card.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[bot3Card.get(i).getValue() + 1], 700, 300 - 10 * bot3Card.size() + 20 * i, 30,
						20, 40, 50, 1, 1, 90);
			}
			for (int i = bot3ChoosedCard.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[bot3ChoosedCard.get(i).getValue() + 1], 625,
						300 - 10 * bot3ChoosedCard.size() + 20 * i, 30, 20, 40, 50, 1, 1, 90);
			}
		} else {
			for (int i = bot3Card.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[0], 700, 300 - 10 * bot3Card.size() + 20 * i, 30, 20, 40, 50, 1, 1, 90);
			}
			for (int i = bot3ChoosedCard.size() - 1; i >= 0; i--) {
				batch.draw(textureRegion[0], 625, 300 - 10 * bot3ChoosedCard.size() + 20 * i, 30, 20, 40, 50, 1, 1, 90);
			}
		}
		batch.end();
	}
	...
	public void renderField(float delta) {
		...
		for (int i = 0; i < field.size(); i++) {
			batch.draw(textureRegion[field.get(i).getValue() + 1], 400 - 20 * field.size() + 40 * i, 275, 0, 0, 40, 50,
					1, 1, 0);
		}
		batch.end();
	}
